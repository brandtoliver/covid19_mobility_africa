...
def run(country, iso, city, adm_region="adm1", adm_kommune="adm2"):
    ...
    def load_prepare_tile(path, iso):
        ...
        data["source_tile"] = [
            f"{lat},{lon}"
            for lat, lon in zip(np.array(slat).round(3), np.array(slon).round(3))
        ]
        data["target_tile"] = [
            f"{lat},{lon}"
            for lat, lon in zip(np.array(elat).round(3), np.array(elon).round(3))
        ]
    ...
    def defaultify(d, depth=0):
        if isinstance(d, dict):
            if depth == 0:
                return defaultdict(
                    lambda: defaultdict(
                        lambda: defaultdict(lambda: defaultlist(lambda: [0, 0]))
                    ),
    ...
    def update_data_out(kommune, idx, data_, data_pop):
        ...
        for (
            ...
            neighbors
        ):  # putting `kommune` in here as a nighbor to `kommune` so if it has 0 within flow then that will show in the output data

            # Compute how many people that live in `kommune` and work in `neighbor`
            flow_from_neighbor = data_kommune_is_target.loc[
                data_kommune_is_target.source_kommune == neighbor
            ].sum()
            data_out[kommune][neighbor]["baseline"][idx][0] = (
                flow_from_neighbor.n_baseline / data_pop.loc[kommune, "n_baseline"]
            )
            data_out[kommune][neighbor]["crisis"][idx][0] = (
                flow_from_neighbor.n_crisis / data_pop.loc[kommune, "n_crisis"]
            )
            ...
            data_out[kommune]["_" + kommune]["baseline"][idx][0] += data_out[kommune][
                neighbor
            ]["baseline"][idx][0]
            data_out[kommune]["_" + kommune]["crisis"][idx][0] += data_out[kommune][
                neighbor
            ]["crisis"][idx][0]
        ...
        for neighbor in neighbors:
            ...
            flow_to_neighbor = data_kommune_is_source.loc[
                data_kommune_is_source.target_kommune == neighbor
            ].sum()
            data_out[kommune][neighbor]["baseline"][idx][1] = (
                flow_to_neighbor.n_baseline / data_pop.loc[neighbor, "n_baseline"]
            )
            data_out[kommune][neighbor]["crisis"][idx][1] = (
                flow_to_neighbor.n_crisis / data_pop.loc[neighbor, "n_crisis"]
            )
            ...
            data_out[kommune]["_" + kommune]["baseline"][idx][1] += data_out[kommune][
                neighbor
            ]["baseline"][idx][1]
            data_out[kommune]["_" + kommune]["crisis"][idx][1] += data_out[kommune][
                neighbor
            ]["crisis"][idx][1]
    ...
    # Data out mobile
    path_boo = os.path.exists(
        f"{PATH_OUT}{country}_movements_between_admin_regions.json"
    )
    ## if path_boo:
    ##     with open(
    ##         f"{PATH_OUT}{country}_movements_between_admin_regions.json", "r"
    ##     ) as fp:
    ...
    data_out = defaultdict(
        lambda: defaultdict(lambda: defaultdict(lambda: defaultlist(lambda: [0, 0])))
    )
    start = 0
    ...
    # Filenames
    fn_days_tile = sorted(
        set([fn[:-9] for fn in os.listdir(PATH_IN_TILE) if fn.endswith(".csv")])
    )
    fn_days_admin = sorted(
        set([fn[:-9] for fn in os.listdir(PATH_IN_ADMIN) if fn.endswith(".csv")])
    )
    ...
    # Loop
    for idx, fn_day in tqdm(
        enumerate(fn_days_tile[start:end_idx], start),
        total=len(fn_days_tile[start:end_idx]),
    ):

        # Get weekday
        dt_obj = dt.datetime(
            year=int(fn_day[-10:-6]), month=int(fn_day[-5:-3]), day=int(fn_day[-2:])
        )
        ...
        ###### OLIVER ######
        data_tile["start_quadkey"] = "A" + data_tile.start_quadkey.astype("int").astype(
            "str"
        )
        data_tile["end_quadkey"] = "A" + data_tile.end_quadkey.astype("int").astype(
            "str"
        )
        ###### OLIVER END ######
        ...
        # Relabel
        data_tile["source_kommune"] = data_tile["start_" + "quadkey"]
        data_tile["target_kommune"] = data_tile["end_" + "quadkey"]
        ...
        ## data_tile = data_tile.loc[
        ##     data_tile["source_kommune"] == data_tile["target_kommune"]
        ## ]

        # Keep only between-municipality flow in admin data
        ## data_admin = data_admin.loc[
        ##     data_admin["source_kommune"] != data_admin["target_kommune"]
        ## ]
        ...
        data_nn = data.loc[
            (data.source_kommune.notnull()) & (data.target_kommune.notnull())
        ]
        kommunes = sorted(
            set(data_nn.start_quadkey.tolist() + data_nn.end_quadkey.tolist())
        )
        ...
        data_pop = (
            data[["target_kommune", "n_crisis", "n_baseline"]]
            .groupby("target_kommune")
            .sum()
        )
